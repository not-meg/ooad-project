...
public class RegisterController {
    ...
    @FXML private TextField srn1, srn2, srn3, srn4;
    @FXML private TextField fullName1, email1, fullName2, email2, fullName3, email3, fullName4, email4;
    ...
    public void initialize() {
        ...
        srn1.textProperty().addListener((observable, oldValue, newValue) -> fetchStudentDetails(newValue, fullName1, email1, gender1));
        srn2.textProperty().addListener((observable, oldValue, newValue) -> fetchStudentDetails(newValue, fullName2, email2, gender2));
        srn3.textProperty().addListener((observable, oldValue, newValue) -> fetchStudentDetails(newValue, fullName3, email3, gender3));
        srn4.textProperty().addListener((observable, oldValue, newValue) -> fetchStudentDetails(newValue, fullName4, email4, gender4));
    }

    private void fetchStudentDetails(String srn, TextField fullNameField, TextField emailField, Label genderLabel) {
        if (srn.isEmpty()) {
    ...
    private boolean validateFields() {
        if (srn1.getText().isEmpty() || srn2.getText().isEmpty() || srn3.getText().isEmpty() || srn4.getText().isEmpty()) {
